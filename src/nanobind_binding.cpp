...
{
IK::Point_3
point_at (IK::Vector_3 (&box)[5], const double &s, const double &t, const double &c)
{
    return IK::Point_3 (box[0].x () + s * box[1].x () + t * box[2].x () + c * box[3].x (), box[0].y () + s * box[1].y () + t * box[2].y () + c * box[3].y (),
...
void
get_corners (IK::Vector_3 (&box)[5], CGAL_Polyline &corners)
{
...
/////////////////////////////////////////////////////////////////////////////////////////////////
IK::Point_3
middle_point (const IK::Point_3 &a, const IK::Point_3 &b)
{
    return IK::Point_3 ((a.x () + b.x ()) / 2, (a.y () + b.y ()) / 2, (a.z () + b.z ()) / 2);
}

void
rtree (std::vector<std::vector<IK::Point_3> > &input_polyline_pairs, std::vector<std::vector<int> > &elements_neighbours, std::vector<std::vector<IK::Point_3> > &elements_AABB,
...
get_connection_zones (
    // input
    std::vector<std::vector<IK::Point_3> > &input_polyline_pairs, std::vector<std::vector<IK::Vector_3> > &input_insertion_vectors, std::vector<std::vector<int> > &input_joint_types,
    std::vector<std::vector<int> > &input_three_valence_element_indices_and_instruction, std::vector<int> &input_adjacency, std::vector<double> &input_joint_parameters_and_types, int &input_search_type,
    std::vector<double> &input_scale, int &input_output_type,
    // output
    std::vector<std::vector<std::vector<IK::Point_3> > > &output_plines, std::vector<std::vector<wood::cut::cut_type> > &output_types,
    // global_parameters
...
)
{
    ...
    wood::main::get_connection_zones (
        ...
        // Global Parameters
        wood::GLOBALS::JOINTS_PARAMETERS_AND_TYPES, input_scale, input_search_type, wood::GLOBALS::OUTPUT_GEOMETRY_TYPE, 0);
}
...
void
read_xml_polylines (std::string &foldername, std::string &filename_of_dataset, std::vector<std::vector<double> > &polylines_coordinates)
{
    // set file paths
    wood::GLOBALS::DATA_SET_INPUT_FOLDER = foldername; // =
...
void
read_xml_polylines_and_properties (std::string &foldername, std::string &filename_of_dataset, std::vector<std::vector<double> > &input_polyline_pairs_coord,
                                   std::vector<std::vector<double> > &input_insertion_vectors_coord, std::vector<std::vector<int> > &input_joints_types,
...
void
closed_mesh_from_polylines (std::vector<std::vector<IK::Point_3> > &vector_of_polyline, std::vector<IK::Point_3> &out_vertices, std::vector<IK::Vector_3> &out_normals, std::vector<std::vector<int> > &out_triangles)
{
...
void
joints (std::vector<std::vector<IK::Point_3> > &input_polyline_pairs, int &search_type, std::vector<std::vector<int> > &element_pairs, std::vector<std::vector<IK::Point_3> > &joint_areas, std::vector<int> &joint_types)
{
...
/////////////////////////////////////////////////////////////////////////////////////////////////
void
bind_wood_nano_types (nb::module_ &m)
{
    ...
    nb::class_<std::vector<wood::cut::cut_type> > (m, "cut_type1")
        ...
        .def ("emplace_back", [] (std::vector<wood::cut::cut_type> &v, wood::cut::cut_type cut_type) { v.emplace_back (cut_type); });
    ...
    nb::class_<std::vector<std::vector<wood::cut::cut_type> > > (m, "cut_type2")
        ...
        .def ("emplace_back", [] (std::vector<std::vector<wood::cut::cut_type> > &v, const std::vector<wood::cut::cut_type> &p) { v.emplace_back (p); });
...
NB_MODULE (wood_nano_ext, m)
{
    ...
    m.def ("inspect", [] (nb::ndarray<> a) {
        ...
        printf ("Device ID = %u (cpu=%i, cuda=%i)\n", a.device_id (), int (a.device_type () == nb::device::cpu::value), int (a.device_type () == nb::device::cuda::value));
        printf ("Array dtype: int16=%i, uint32=%i, float32=%i\n", a.dtype () == nb::dtype<int16_t> (), a.dtype () == nb::dtype<uint32_t> (), a.dtype () == nb::dtype<float> ());
    });
    ...
    m.def ("get_connection_zones", &get_connection_zones, "input_polyline_pairs" _a, "input_insertion_vectors" _a, "input_joint_types" _a, "input_three_valence_element_indices_and_instruction" _a, "input_adjacency" _a,
           "input_joint_parameters_and_types" _a, "input_search_type" _a, "input_scale" _a, "input_output_type" _a, "output_plines" _a, "output_types" _a, "input_joint_volume_parameters" _a,
    ...
    m.def ("read_xml_polylines", &read_xml_polylines, "foldername" _a, "filename_of_dataset" _a, "polylines_coordinates" _a, "This function reads xml polylines.");

    m.def ("read_xml_polylines_and_properties", &read_xml_polylines_and_properties, "foldername" _a, "filename_of_dataset" _a, "input_polyline_pairs_coord" _a, "input_insertion_vectors_coord" _a, "input_joints_types" _a,
           "input_three_valence_element_indices_and_instruction" _a, "input_adjacency" _a, "This function reads xml polylines and properties.");

    m.def ("closed_mesh_from_polylines", &closed_mesh_from_polylines, "vector_of_polyline" _a, "out_vertices" _a, "out_normals" _a, "out_triangles" _a, "This function creates a closed mesh from polylines.");

    m.def ("joints", &joints, "input_polyline_pairs" _a, "search_type" _a, "element_pairs" _a, "joint_areas" _a, "joint_types" _a, "This function gets joints.");

    m.attr ("joint_parameters_and_types") = wood::GLOBALS::JOINTS_PARAMETERS_AND_TYPES;

    m.def ("mesh_boolean_difference_from_polylines", &cgal::mesh_boolean::mesh_boolean_difference_from_polylines, "input_plines" _a, "output_plines" _a, "out_vertices" _a, "out_normals" _a, "out_triangles" _a,
           "This function creates a mesh boolean difference from polylines.");

    m.def ("beam_volumes", &wood::main::beam_volumes, "polylines" _a, "polylines_segment_radii" _a, "polylines_segment_direction" _a, "allowed_types" _a, "min_distance" _a, "volume_length" _a, "cross_or_side_to_end" _a,
           "flip_male" _a, "index_polylines" _a, "index_polylines_segment" _a, "distance" _a, "point_pairs" _a, "volume_pairs" _a, "joints_areas" _a, "joints_types" _a, "output_plines" _a, "output_types" _a,
           "compute_joints" _a, "division_distance" _a, "shift" _a, "output_type" _a, "use_eccentricities_to_scale_joints" _a, "This function computes beam volumes.");
}