...
def _unpack(rm, beam_offsets: list[float] | None):
    beam_dicts = list(rm.beams)
    side0_raw  = [list(p) for p in rm.side0]
    side1_raw  = [list(p) for p in rm.side1]
...
def reciprocal_move_elements(
    ...
    return _unpack(rm, beam_offsets)
