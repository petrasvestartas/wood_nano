...
def joinery_solver_elements(
...
) -> tuple[list[JoineryElementCompas], list[JointResultCompas]]:
    ...
    elements = [JoineryElementCompas(d) for d in raw["elements"]]
    joints   = [JointResultCompas(d)   for d in raw["joints"]]
