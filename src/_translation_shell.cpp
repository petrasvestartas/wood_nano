...
NB_MODULE(_translation_shell, m) {
    // Import _wood_element so WoodElement is registered once in nanobind's
    // global type registry — returning ts.elements works without re-registering.
    nb::module_::import_("wood_nano._wood_element");

    nb::class_<TranslationShell>(m, "TranslationShell")
        .def(nb::init<>())
        .def_prop_ro("mesh",     [](const TranslationShell& ts) { return mesh_to_dict(ts.mesh); })
        .def_prop_ro("elements", [](const TranslationShell& ts) { return ts.elements; });

    m.def("make_translation_shell",
        [](const Pts& cs, const Pts& pr, double thickness, double chamfer, double chamfer_angle) {
            return TranslationShell(pts_to_polyline(cs), pts_to_polyline(pr),
    ...
    m.def("make_default_translation_shell",
        [](double thickness, double chamfer, double chamfer_angle) {
            return TranslationShell(TranslationShell::default_cross_section(),
